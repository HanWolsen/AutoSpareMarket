...
// ── PRODUCTS ─────────────────────────────────────────────────
let productsData=[];
async function loadProducts() { const b=document.getElementById('productsTbody'); loading(b,6); try{ productsData=await api('/products')||[]; renderProducts(productsData); }catch(e){ empty(b,6,`Ошибка: ${e.message}`); } }
function renderProducts(list) { const b=document.getElementById('productsTbody'); if(!list.length){empty(b,6);return;} b.innerHTML=list.map(p=>`<tr><td><span class="id-badge">${p.id}</span></td><td><strong>${esc(p.name)}</strong></td><td class="muted-cell">${esc(p.description||'—')}</td><td><span class="cell-badge">${p.warehouseCellId??'—'}</span></td><td class="muted-cell">${fmtDate(p.dateAdd)}</td><td><div class="row-actions"><button class="btn-icon" onclick="editProduct(${p.id})">✏</button><button class="btn-icon danger" onclick="deleteProduct(${p.id})">✕</button></div></td></tr>`).join(''); }
...
document.getElementById('productForm').addEventListener('submit',async e=>{ e.preventDefault(); const f=e.target; const id=f.id.value; const dto={warehouseCellId:Number(f.warehouseCellId.value),name:f.name.value.trim(),description:f.description.value.trim()||null}; try{ if(id){await api(`/products/${id}`,{method:'PUT',body:JSON.stringify({id:Number(id),...dto})}); showToast('Товар обновлён');}else{await api('/products',{method:'POST',body:JSON.stringify(dto)}); showToast('Товар создан');} closeModal('productModal'); loadProducts(); }catch(err){ showToast(err.message,'error'); } });
async function editProduct(id){ const p=productsData.find(x=>x.id===id); if(!p)return; const f=document.getElementById('productForm'); document.getElementById('productModalTitle').textContent='Редактировать товар'; f.id.value=p.id; f.warehouseCellId.value=p.warehouseCellId; f.name.value=p.name; f.description.value=p.description||''; openModal('productModal'); }
